...
export default function TestNotifications() {
  ...
  const testNotifications = [
    {
      ...
      icon: Mail,
      action: () => notifyNewMessage("Test User", "Hello! This is a test message"),
      color: "blue"
    },
    {
    ...
    },
    {
      ...
      icon: Bell,
      action: () => notifyAIMatch("Alex Rodriguez", "Shared interests in design and tech"),
      color: "orange"
    },
    {
  ...
  const handleTestNotification = async (testFn: () => Promise<any>, testName: string) => {
    setIsLoading(true);
    try {
      await testFn();
  ...
  return (
    <LayoutNew>
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">Test Notifications</h1>
        ...
        <div className="card-elevated p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Notification Support Status</h2>
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${notificationSupport.supported ? 'bg-green-500' : 'bg-red-500'}`}></div>
              <span className="text-sm">
                Browser Support: {notificationSupport.supported ? 'Supported' : 'Not Supported'}
              </span>
            </div>
            <div className="flex items-center gap-3">
          ...
          {notificationSupport.permission !== 'granted' && (
            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                <strong>Note:</strong> You need to enable notifications first using the bell icon in the top navigation.
              </p>
            </div>
          )}
        </div>
          ...
          {testNotifications.map((test, index) => (
            <div key={index} className="card-elevated p-6">
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${getColorClasses(test.color)}`}>
                ...
                <div className="flex-1">
                  ...
                  <button
                    onClick={() => handleTestNotification(test.action, test.title)}
                    disabled={isLoading || notificationSupport.permission !== 'granted'}
        ...
        <div className="card-elevated p-6">
          ...
          <ol className="space-y-2 text-sm text-gray-600">
            <li>1. Click the bell icon in the navigation to enable notifications</li>
          ...
          <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>Tip:</strong> Each notification type includes specific data that will navigate you to the relevant page when clicked.
            </p>
          </div>
        </div>
      </div>
